...
function get_reserve($con, $user_id, $trans_no) {
    ...
    $sql = "SELECT * FROM reservation INNER JOIN `cottage/hall` ON `cottage/hall`.id = reservation.`cottage/hall_id` INNER JOIN `user` ON `user`.user_id = reservation.customer_id
            INNER JOIN user_type ON user_type.user_type_id = `user`.user_type_id WHERE `user_id` = '$user_id' AND `trans_no` = '$trans_no' AND `status` = 'Processing'";
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        echo "<div class='row'>
                <div class='col-md-4'>
                <a href='#' data-toggle='modal' data-target='#modal-customer'><img src='image/edit.png'> Edit Details</a>
                    <table>
                        <tr>
                            <th>Name:</th>
        ...
        while ($fetch = mysqli_fetch_assoc($query)) {
            ...
            echo "<tr>
                   ...
                   <td class='text-right'><a href='#' data-toggle='modal' data-target='#modal-".$fetch["id_res"]."' class='btn btn-warning btn-sm'><img src='image/edit-w.png'></a>

                    <button type='button' data-toggle='modal' data-target='#modalDel-".$fetch['id_res']."' class='btn btn-danger btn-sm'><img src='image/delete-w.png'></button></td>
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        while ($fetch = mysqli_fetch_assoc($query)) {
            ...
            echo "<div class='swiper-slide'>
                        ...
                        <h5 class='card-title border-bottom pb-3'><b>".$fetch['name']."</b><a href='#' class='float-right d-inline-flex share'></a></h5>
...
function get_my_res($con, $user_id) {
    ...
    $sql = "SELECT *, `reservation`.id as rec_id FROM reservation INNER JOIN `cottage/hall` ON `cottage/hall`.id = reservation.`cottage/hall_id`
            INNER JOIN `user` ON `user`.user_id = reservation.customer_id INNER JOIN user_type ON user_type.user_type_id = `user`.user_type_id WHERE user_id = '$user_id'";
    $query = mysqli_query($con, $sql);
...
function get_pending($con, $userid) {
    ...
    $sql = "SELECT *, `reservation`.id_res as res FROM reservation INNER JOIN `cottage/hall` ON `cottage/hall`.id = reservation.`cottage/hall_id`
            INNER JOIN `user` ON `user`.user_id = reservation.customer_id INNER JOIN user_type ON user_type.user_type_id = `user`.user_type_id 
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        while ($fetch = mysqli_fetch_assoc($query)){
            ...
            echo "
                 <tbody>
                  ...
                  <td><buton type='button' data-toggle='modal' data-target='#modal-view-".$fetch['res']."' class='btn btn-success view' id='".$fetch['trans_no']."'>View</buton></td>
            ...
            echo "<div class='modal fade in' id='modal-view-".$fetch['res']."'>
                ...
                <div class='modal-content'>
                    ...
                    if ($fetch['status'] != "Cancelled") {
                        echo "<a href='#' class='btn btn-danger' data-toggle='modal' data-target='#modalconfirm".$fetch['trans_no']."'>Cancel Reservation</a>";
                    }else {
                        echo "<a href='#' class='btn btn-danger' data-toggle='modal' data-target='#modalconfirm".$fetch['trans_no']."'>Delete</a>";
                    }

                    echo "<button type='button' class='btn bg-maroon' data-dismiss='modal'>Close</button>
        ...
        echo "<tr> <td colspan='6'><center>No Reservation Yet</center></td> </tr>";
